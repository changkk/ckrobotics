  ...
  void imageCb(const sensor_msgs::ImageConstPtr& msg)
{
    ...
    if(ok2)
    {
        rectangle(img, multiTracker->getObjects()[0], Scalar( 255, 0, 0 ), 2, 1);
    }
...
static void onMouse( int event, int x, int y, int, void* )
{
    ...
    if ( event == cv::EVENT_LBUTTONDOWN ) {
        ...
        ok = tracker->init(img, tmp);
        //rectangle(img, Point(x-10,y-10), Point(x+10,y+10), CV_RGB(255, 0, 0), 0.5 );

        Ptr<Tracker> tmp_tracker = TrackerMIL::create();
        multiTracker->add(tmp_tracker, img, Rect2d(tmp));
    }
