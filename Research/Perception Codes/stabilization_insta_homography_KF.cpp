...
public:
    ...
    void processImage(Mat& img) {
        ...
        Mat stab_mask(img.rows, img.cols, CV_8UC1, cv::Scalar(0));
        Mat stab_roi(stab_mask, cv::Rect(0,0,1440,1440));
        stab_roi = Scalar(255);  

        if(trackedFeatures.size() < 200) {
            goodFeaturesToTrack(gray,corners,300,0.01,10,stab_mask);
        ...
        if(!prevGray.empty()) {
			...
			// {
			// 	circle(img, Point(trackedFeatures.at(i).x,trackedFeatures.at(i).y), 2, Scalar(0, 0, 255), -1); //red(past)
			// 	circle(img,  Point(corners.at(i).x,corners.at(i).y), 2, Scalar(255, 0, 0), -1); //blue(current)
			// 	line(img, Point(trackedFeatures.at(i).x,trackedFeatures.at(i).y), Point(corners.at(i).x,corners.at(i).y), Scalar(0,255,0), 1);
            // }
