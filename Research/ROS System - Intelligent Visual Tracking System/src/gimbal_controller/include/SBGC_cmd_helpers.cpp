...
//---------------------------------------------------
void SBGC_cmd_control_pack_rtrq(SBGC_cmd_control_t &p, SerialCommand &cmd) {
	cmd.init(85); //Data interval request command as message id
	cmd.writeByte(25); //Send extended realtime data
	cmd.writeByte(20); //At 40 hz (every 25 ms)
	for ( int g = 0; g < 19 ; g++) //Writing bytes automatically increase cmd.len variable, no need to do it manually.
	{
		cmd.writeByte(0);
	}
	// cmd.len = 21;
	// std::cout << std::endl << "length is: "<< (int)cmd.len << std::endl << std::endl;
	// for(int h = 0; h < cmd.len; h++ )
	// {
	// 	std::cout << (int)cmd.data[h] << "	";
	// }
	// std::cout << std::endl;
}
//--------------------------------------------------
