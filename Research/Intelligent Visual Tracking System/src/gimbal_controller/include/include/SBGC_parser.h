...
public:
	...
	inline void onParseError(uint8_t error = PARSER_ERROR_PROTOCOL) { 
		parser_error_count++; 
		// std::cout << "PARSE ERROR! ERROR COUNTER:	" << parser_error_count << std::endl;
	}
	...
	inline int8_t read_cmd() {
		while(com_obj->getBytesAvailable()) {
			// std::cout << "barwtgf" << std::endl;
			if(process_char(com_obj->readByte())) {
				// std::cout << "wtfbar" << std::endl;
				return 1;
			}
		}
	...
	uint8_t send_command(uint8_t cmd_id, void *data, uint16_t size, uint8_t wait = 1) {
		if(com_obj != NULL && size <= (SBGC_CMD_MAX_BYTES - SBGC_CMD_NON_PAYLOAD_BYTES)) {
			if(wait || com_obj->getOutEmptySpace() >= size + SBGC_CMD_NON_PAYLOAD_BYTES) {
				...
				// std::cout << std::endl << "length is: "<< (int)size << std::endl << std::endl;
				// for(int h = 0; h < size; h++ )
				// {	
				// 	std::cout << (int)(((uint8_t*)data)[h]) << "	";
				// }
				// std::cout << std::endl;
	...
	//--------------------------------------------------------
	// 	uint8_t send_command_rtrq(uint8_t cmd_id, void *data, uint16_t size, uint8_t wait = 1) {
	// 	if(com_obj != NULL && size <= (SBGC_CMD_MAX_BYTES - SBGC_CMD_NON_PAYLOAD_BYTES)) {
	// 		if(wait || com_obj->getOutEmptySpace() >= size + SBGC_CMD_NON_PAYLOAD_BYTES) {
				
	// 			std::cout << std::endl << "length is: "<< (int)size << std::endl << (int)((uint8_t)25) << std::endl;
	// 			for(int h = 0; h < size; h++ )
	// 			{	
	// 				std::cout << (int)(((uint8_t*)data)[h]) << "	";
	// 			}
	...
	// 				// if ( i == 0)
	// 				// {
	// 				// 	com_obj->writeByte((uint8_t)25);
	// 				// 	SerialCommand::update_checksum(checksum, ((uint8_t)25));
	// 				// }
	// 				// else if ( i == 1)
	// 				// {
	// 				// 	com_obj->writeByte((uint8_t)25);
	...
	// 				// {
	// 				// 	com_obj->writeByte(0);
	// 				// 	SerialCommand::update_checksum(checksum, (0));
	// 				// }
	// 				com_obj->writeByte(((uint8_t*)data)[i]);
	// 				SerialCommand::update_checksum(checksum, ((uint8_t*)data)[i]);
	...
	*/
	...
	//-------------------------------------------------------------
	// 	inline uint8_t send_cmd_rtrq(SerialCommand &cmd, uint8_t wait = 1) {
	// 	return send_command_rtrq(cmd.id, cmd.data, cmd.len, wait);
	// }
	//-------------------------------------------------------------
