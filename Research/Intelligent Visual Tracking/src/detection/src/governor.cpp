...
double pitch_ce = 0, yaw_ce = 0, omni_angles[3] = {0, 0, 0}, gim_angles[3] = {0, 0, 0};
double elev = 0, azim = 0, elev_old = 0, azim_old = 0, t = 0, t_old = 0, dt = 0, max_ang_spd = 1;
double targ_bearing[2] = {0, 0};
...
void periph_detec_cb(const geometry_msgs::PolygonStamped msg)
{
    ...
    // //Detection box offset from center based control
    ...
    // Set setpoint to half of image widht/height (center)
    tmp_msg.data = targ_bearing[1];
    yaw_sp_pub.publish(tmp_msg);
    tmp_msg.data = targ_bearing[0];
    pitch_sp_pub.publish(tmp_msg);

    // Set state to filter input
    tmp_msg.data = gim_angles[2]; //Gimbal current yaw angle
    yaw_st_pub.publish(tmp_msg);
    tmp_msg.data = gim_angles[0]; //Gimbal current pitch angle
    pitch_st_pub.publish(tmp_msg);
...
void gimbal_angles_cb(const sensor_msgs::Imu msg)
{
    Quaternion q(msg.orientation.x, msg.orientation.y, msg.orientation.z, msg.orientation.w);
    Matrix3x3 m(q);
    m.getRPY(gim_angles[0], gim_angles[1], gim_angles[2]);
...
int main(int argc, char **argv)
{
    ...
    // ros::Subscriber periph_detec = ic.n.subscribe("/gimbal_imu_angles",1, gimbal_angles_cb);
